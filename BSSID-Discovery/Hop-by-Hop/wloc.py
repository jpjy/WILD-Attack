...
def ListWifiDepuisApple(wifi_list):
	...
	for wifi in wifi_list.wifi:
		...
		if wifi_list.HasField('valeur_inconnue2'):
			print('Inconnu2 : ', '%X' % wifi_list.valeur_inconnue2)
...
def QueryBSSID(query, more_results=True):
	...
	if isinstance(query, str):
		...
		raise TypeError('Provide 1 BSSID as a string or multiple BSSIDs as a list of strings')
	...
	data = b"\x00\x01\x00\x05" + b"en_US" + b"\x00\x13" + b"com.apple.locationd" + \
       b"\x00\x0a" + b"8.1.12B411" + b"\x00\x00\x00\x01\x00\x00\x00" + \
       bytes([longueur_chaine_liste_wifi]) + chaine_liste_wifi
...
def QueryMobile(cellid,LTE=False):
	...
	headers = {'Content-Type':'application/x-www-form-urlencoded', 'Accept':'*/*', "Accept-Charset": "utf-8","Accept-Encoding": "gzip, deflate",\
			"Accept-Language":"en-us", 'User-Agent':'locationd/1753.17 CFNetwork/711.1.12 Darwin/14.0.0'}	
	data = b"\x00\x01\x00\x05" + b"en_US" + b"\x00\x13" + b"com.apple.locationd" + \
       b"\x00\x0c" + b"7.0.3.11B511" + b"\x00\x00\x00\x01\x00\x00\x00" + \
       bytes([len(req_string)]) + req_string
